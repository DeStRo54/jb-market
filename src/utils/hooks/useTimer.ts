...
export const useTimer = ((...params: any[]) => {
  const initialSeconds = Math.max(
    (params[0] ?? 0) as PositiveInteger<number>,
    0,
  );
  const options = (
    typeof params[1] === "object" ? params[1] : { onExpire: params[1] }
  ) as UseTimerOptions | undefined;

  const [active, setActive] = useState(
    initialSeconds > 0 && (options?.immediately ?? true),
  );
